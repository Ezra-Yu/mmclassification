...
class EMAHook(BaseEMAHook):
    ...
    Args:
        ...
        evaluate_on_nonema (bool): Whether to evaluate (validate and test)
            on non-EMA model during val-loop and test-loop. Defaults to False.
    ...
    def before_test_epoch(self, runner) -> None:
        ...
        if self.evaluate_on_ema:
            ...
            MMLogger.get_current_instance().info(
                'Start testing on non-EMA model.')
    ...
    def after_test_epoch(self,
        ...
        if self.evaluate_on_ema and self.evaluate_on_nonema:
            ...
            # compute metrics
            nonema_metrics = test_loop.evaluator.evaluate(
                len(test_loop.dataloader.dataset))

            for k, v in nonema_metrics.items():
                runner.message_hub.update_scalar(f'test/{k}_non-ema', v)
