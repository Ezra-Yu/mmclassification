...
class ProgressiveCurriculumHook(Hook):
    ...
    def before_train(self, runner):
        ...
        print_log(
            'There is no `RandomResizedCrop` in your training pipelie,'
    ...
    def before_train_epoch(self, runner: Runner) -> None:
        ...
        """
        if isinstance(runner.train_loop, EpochBasedTrainLoop) and (
                self.rrc_index and runner.epoch in self.curriculum_cfg):
            transforms = (
                runner.train_loop.dataloader.dataset.pipeline.transforms)
            transforms[self.rrc_index].scale = self.curriculum_cfg[
                runner.epoch]
    ...
    def before_train_iter(self,
        ...
        Args:
            ...
            data_batch (Sequence[dict], optional): Data from dataloader.
                Defaults to None.
        """
        if isinstance(runner.train_loop, IterBasedTrainLoop) and (
                self.rrc_index and runner.iter in self.curriculum_cfg):
            transforms = (
                runner.train_loop.dataloader.dataset.pipeline.transforms)
            transforms[self.rrc_index].scale = self.curriculum_cfg[
                runner.epoch]
